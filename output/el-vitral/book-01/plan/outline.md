# Outline — visible timeline

> This file is the **public** timeline: every chapter, what the reader
> witnesses. The hidden truth lives in `shadow.md`. The seeds catalog
> lives in `seeds.md`.
>
> For each chapter: **Plot beats** (events) · **Texture beats** (2-4
> dwelling moments, 300-500 words) · **Subtext beats** (interior / unsaid /
> seeds active). Target length per chapter: 8000-12000 words.
>
> **Reparto propuesto (libre, grimorio §9 — editable):** Bruno ~13 años, huérfano,
> criado en el oficio de la madera (su padre, carretero, muerto); la madre tejedora
> (el telar, ausente). **Sela** = niña de Vega Parda a la que salva (cap 1).
> **Brígida** = la comadre. **Olmo** = aldeano verde, POV del cap 3.
> **Tilo / Mirta** = vecinos verdes [FIJO].

# Act 1 — Inhabitación (ch 1-7)

> Vega Parda (1-3) → el camino → Altaluz (4-7). Slow-immersion: inhabitar el oficio,
> la magia cotidiana, el apagado como textura. Se monta el refugio que el 13 invierte.

## Chapter 1
- **Title:** El pozo sin brocal
- **POV:** Bruno
- **Where / when:** Vega Parda, un día de convoy (apertura)
- **Target words:** 8000-12000
- **Function in the act:** la promesa del cap 1 (un niño invisible que se oculta imitando
  a su padre, en un mundo cuyo color se apaga) **y** el inciting incident: el destello de
  blanco frío que Mauro presencia y que le hace mirar la cara de Bruno.

### Plot beats
- Amanece: Bruno solo en la casa vacía, el telar mudo de la madre presidiendo el cuarto.
  Se mueve haciéndose pequeño por hábito aunque no haya nadie que lo mire.
- Sale al pueblo con un recado; corre la voz: este año el carro de la cosecha sube
  hasta la vega. Miedo de aldea. Bruno mide a la gente por las manos, se borra en un rincón.
- El convoy pasa: el oficiante **lee** a la fila con el prisma; a quien miden se apaga;
  queda polvo gris en el umbral.
- Sela, una cría, se asoma al pozo sin brocal y cae.
- Sin querer, Bruno da el **destello de blanco frío** —la luz que no calienta— y la salva.
- Mauro, un forastero gris que mira caras donde todos miran color, lo ve. **Le mira la cara.**

### Texture beats
- La casa al alba, Bruno solo: el telar mudo de la madre, el hilo verde a medio tejer
  yéndose al gris; el gesto heredado del padre (hacerse pequeño) como única lengua.
- El paso del prisma: el rito como burocracia temible y cotidiana; el polvo gris.
- La moneda del santo que circula en el mercado ("el santo vendrá", como quien dice que lloverá).

### Subtext beats
- Bruno se borra como su padre (postura de asa de cubo) sin saber de qué heredó el esconderse.
- Espera a que los ojos de la gente le caigan a las manos; nunca le caen.
- Tras el destello se le cae un recuerdo menudo (un caldo) que no nota.
- Seeds activos: `el-blanco-frio`(plant), `el-convoy-jovenes`(plant), `el-prisma-lee-drena`(plant),
  `el-polvo-gris`(plant), `el-caldo`(plant), `la-moneda-del-santo`(plant), `el-telar-hilo-verde`(plant),
  `ojos-que-no-suben`(plant), `borrarse-como-el-padre`(plant).

### Transition out
Mauro no aparta los ojos de su cara. Por primera vez en su vida, a Bruno lo han visto.

## Chapter 2
- **Title:** Lo que se mide
- **POV:** Bruno
- **Where / when:** Vega Parda, día siguiente
- **Target words:** 8000-12000
- **Function in the act:** Mauro **confirma su dato** observando a Bruno; el rito del
  prisma se ve entero; Mauro decide llevárselo arriba.

### Plot beats
- Mauro se queda con un pretexto (esperar el carro, arreglar algo) y ronda a Bruno:
  pequeñas pruebas, le mira las manos.
- El rito del prisma de cerca: la Lectura de la fila; a quien leen bien, se apaga; el
  oficiante anota.
- Bruno entre el taller vacío del padre y el telar mudo de la madre: dos linajes borrados.
- Mauro se ofrece a llevarlo "arriba", a Altaluz, donde la luz es limpia.
- La aldea lo despide entre alivio (una boca menos) y la moneda del santo.

### Texture beats
- El rito del prisma de cerca: el cristal que mide y, luego, deja gris.
- El taller del padre: herramientas con la forma de su mano; lo que enseña un oficio sin palabras.
- Las manos de Mauro cerca del cristal del rito: la mancha pálida que no sale, la soltura de
  quien lo ha hecho mil veces (`las-manos-del-lector` plant, mudo).
- La comadre Brígida liando un cigarro despacio antes de hablar.

### Subtext beats
- Mauro mira la cara de Bruno demasiado; su ternura y su frío conviven sin enfrentarse.
- Bruno confunde ser elegido con ser salvado.
- Mauro se mueve ante el prisma con una familiaridad que no es de un gris cualquiera; el
  lector lo roza sin formularlo (`las-manos-del-lector`).
- Seeds: `el-prisma-lee-drena`(echo, rito en escena), `el-tutor-tiene-agenda`(implícito),
  `borrarse-como-el-padre`(implícito), `las-manos-del-lector`(plant 2, mudo).

### Transition out
Parten al alba hacia arriba; Vega Parda queda gris a su espalda, y el telar dentro.

## Chapter 3
- **Title:** La cosecha
- **POV:** Olmo (aldeano verde)
- **Where / when:** una aldea verde del camino, mismos días
- **Target words:** 8000-12000
- **Function in the act:** subplot A — el agravio verde; mostrar lo que Bruno no puede ver
  (la cosecha desde dentro de una familia verde).

### Plot beats
- Olmo en su día: el verde de los campos, la vida de aldea, los suyos.
- El convoy llega: el prisma lee; se llevan a los jóvenes (entre ellos alguien de Olmo).
- Riña de mercado: un campesino verde, sin querer, deja sin fuerza la orden de un soldado
  magenta. Nadie saca la conclusión en voz alta.
- Tilo y Mirta presentes como vecinos; Tilo murmura, aún no "el líder".
- La aldea queda con menos jóvenes y más miedo.

### Texture beats
- El verde que "antes se oía crecer desde el camino"; el campo después, más callado.
- La carga del convoy; el llanto contenido; las manos que se sueltan.
- El polvo del camino cuando el carro sube hacia Altaluz/Solio.

### Subtext beats
- El verde anula al magenta (Ley 4) asomando como anécdota, sin que nadie lo formule.
- El agravio que arderá en los Libros II-III.
- Seeds: `el-convoy-jovenes`(echo fuerte, POV propio), `el-complementario-verde-magenta`(plant),
  `el-cribado`(implícito).

### Transition out
El carro sube con los jóvenes. Olmo mira el polvo hasta que el camino se lo traga.

## Chapter 4
- **Title:** El camino arriba
- **POV:** Bruno
- **Where / when:** el camino de Vega Parda a Altaluz
- **Target words:** 8000-12000
- **Function in the act:** inhabitar el viaje; el apagado a escala de provincias; sembrar
  la mirada de Mauro que **mide**.

### Plot beats
- Bruno y Mauro suben; cruzan provincias drenadas, grises.
- Mauro le enseña cosas; lo mide sin que parezca; preguntas que son pruebas.
- Cruzan rastros del convoy; Bruno reconoce el polvo gris.
- Llegan al umbral de Altaluz: la luz limpia, arriba.

### Texture beats
- El paisaje apagado, provincia tras provincia; el frío de la altura.
- La comida del camino; el modo de Mauro de partir el pan con las dos manos.
- Las campanas a lo lejos, marcando un tiempo que no es el de la vega.

### Subtext beats
- Bruno se borra como su padre ante los desconocidos (eco).
- Mauro mide; su atención es interés, no afecto, aunque parezca lo segundo.
- Seeds: `el-tutor-tiene-agenda`(plant), `borrarse-como-el-padre`(echo 4), `el-polvo-gris`(implícito).

### Transition out
Altaluz se abre delante, blanca y alta. Bruno cree haber llegado a salvo.

## Chapter 5
- **Title:** La luz limpia
- **POV:** Bruno
- **Where / when:** Altaluz, primeros días
- **Target words:** 8000-12000
- **Function in the act:** instalarse en el refugio; segundo destello menor; el pozo que
  "pide"; el cazador, lejos, empieza a oler el rumor.

### Plot beats
- Bruno se instala: reglas, otros aprendices, la luz dura que no deja sombra.
- Un susto (un accidente menor) le saca otra chispa de blanco frío; un experto lejano
  medio-lo-percibe y lo archiva como suerte.
- Junto a una cisterna de Altaluz, Bruno siente el vacío que "pide" ser llenado.
- Mauro lo aloja, lo instruye, lo vigila.

### Texture beats
- La arquitectura de luz limpia; lo distinto de todo respecto a la vega.
- La cisterna/el pozo de Altaluz; el hueco del pecho de Bruno que tira hacia él.
- La disciplina del lugar; la comida ajena.

### Subtext beats
- El blanco frío archivado como suerte; el pozo que pide, todavía sin opción consciente.
- Se le cae otro recuerdo menudo (el caldo, eco), casi invisible.
- Seeds: `el-blanco-frio`(echo 5), `el-pozo-que-pide`(plant), `el-caldo`(echo 5).

### Transition out
A lo lejos, alguien pregunta por "un blanco que no calienta". Bruno no lo oye.

## Chapter 6
- **Title:** Le miró la cara
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el vínculo — el único momento en que los ojos **suben**.

### Plot beats
- Mauro tutela a Bruno; una lección que es también medición.
- En un momento, Mauro le sostiene la cara y lo mira a los ojos.
- Bruno, por primera vez, es visto; el Need rozado.

### Texture beats
- La sala de Mauro: sus instrumentos (prismas, la Lectura) como muebles de su oficio.
- El gesto de dar y recibir con las dos manos; las campanas a sundown.

### Subtext beats
- El lector debe sentirlo como amor aunque sea medición: ojos que suben, una vez.
- Seeds: `ojos-que-no-suben`(echo 6), `el-tutor-tiene-agenda`(implícito).

### Transition out
Bruno guarda esa mirada como un tesoro. No sabe que fue una lectura.

## Chapter 7
- **Title:** Refugio
- **POV:** Bruno
- **Where / when:** Altaluz, semanas después — cierre de acto 1
- **Target words:** 8000-12000
- **Function in the act:** montar el refugio para invertirlo en el 13; una tregua que el
  lector sabe condenada.

### Plot beats
- Días de rutina; Bruno casi pertenece; aprende el lugar.
- Conserva un objeto de la madre; el telar/hilo verde vuelve como recuerdo (eco).
- Bajo la calma, Mauro mide: una nota fría que el lector capta y Bruno no.

### Texture beats
- La rutina de Altaluz; el objeto de la madre guardado.
- La vidriera de Altaluz que —a diferencia de la de Vega Parda— **no** se apaga, porque
  se nutre de lo robado.

### Subtext beats
- La paz que el lector sabe condenada; Bruno se siente, por primera vez, a salvo.
- Punto de contacto de la subtrama A (mudo): la vidriera de Altaluz no se apaga porque la
  cosecha de abajo la alimenta; Bruno lo roza sin formularlo. (3.er toque de la hebra verde,
  con cap 3 y cap 14.)
- Seeds: `el-telar-hilo-verde`(echo 7), `el-convoy-jovenes`(echo mudo 7).

### Transition out
Por primera vez Bruno duerme sin esconderse. (Y el lector teme por ello.)

# Act 2 — Expansión → vuelco → cerco (ch 8-18 · **midpoint ch 13**)

> El refugio se llena de mundo (8-12), se invierte en el midpoint (13), y se cierra como
> cerco (14-18) hacia el all-is-lost. Nunca dos capítulos de acción seguidos.

## Chapter 8
- **Title:** El cristal que da y quita
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** tier *skilled* — el drenaje por vidrio en escena; medir = robar.

### Plot beats
- Una lección/rito muestra el drenaje por vidrio: cómo se fija color en cristal.
- Bruno ve, sin que se nombre, que el mismo cristal que mide, roba.
- Alguien queda más gris tras el rito.

### Texture beats
- La mecánica del vidrio; el zumbido del color fijado, que calienta como colmena.
- El polvo gris del después; lo cotidiano del horror.

### Subtext beats
- El lector siente la equivalencia (el Negro / el drenaje) sin que se diga.
- La soltura de Mauro ante el cristal del rito asoma otra vez, de dentro (`las-manos-del-lector` eco 8).
- Seeds: `el-prisma-lee-drena`(echo 8), `el-polvo-gris`(echo 8), `el-negro-y-el-drenaje`(plant 8),
  `las-manos-del-lector`(echo 8).

### Transition out
Bruno se mira las manos y no sabe por qué le inquietan.

## Chapter 9
- **Title:** La ascensión
- **POV:** Bruno
- **Where / when:** Altaluz / la Iglesia
- **Target words:** 8000-12000
- **Function in the act:** sembrar el Blanco Falso; reforzar la mentira del santo (decoy).

### Plot beats
- La Iglesia canoniza una "ascensión": un coloreado ardió en blanco y "subió al cielo".
- Altaluz celebra; la hagiografía, las monedas del santo.
- Foil silencioso: el blanco que **calienta** (falso) frente al blanco frío de Bruno —que
  nadie empareja en voz alta.

### Texture beats
- La liturgia; el calor del blanco de combustión; el sermón.
- Las monedas del santo pasando de mano en mano como esperanza.

### Subtext beats
- Propaganda como consuelo; el lector sostiene la esperanza del santo (decoy `el-portaluz`).
- Seeds: `el-blanco-falso`(plant 9), `la-moneda-del-santo`(echo 9), `el-tutor-tiene-agenda`(echo 9).

### Transition out
Bruno reza con los demás por el santo que vendrá.

## Chapter 10
- **Title:** El método
- **POV:** Cándido
- **Where / when:** en ruta, acercándose a Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el método del cazador; su reloj de sospechas; plant del casi-test.

### Plot beats
- Cándido trabaja: el estuche de cristales calibrados; el test del complementario como
  oficio frío y competente.
- Persigue un rumor de blanco-sin-calor; su reloj de sospechas apunta a Altaluz.
- Archiva una anomalía sin nombre.

### Texture beats
- El oficio del cazador: paciente, caro, metódico; los cristales en su estuche.
- Su modo de mirar a la gente como quien lee un libro de cuentas.

### Subtext beats
- Ironía dramática: el lector sabe a quién busca; Cándido cree cazar al Devorador.
- Seeds: `el-cazador-casi-test`(plant 10).

### Transition out
Cándido pone rumbo a Altaluz "de rutina".

## Chapter 11
- **Title:** Amar es peligro
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el vínculo **es** el pecado; un casi-uso peligroso cerca de Mauro.

### Plot beats
- Bruno y Mauro más cerca; en un momento de afecto/defensa, Bruno casi usa lo que es.
- La depredación hacia arriba se hace textura: cerca de Mauro (coloreado), Bruno es peligro.
- Mauro lo protege y lo estudia a la vez.

### Texture beats
- La intimidad de la tutela; el blanco frío que asoma y se traga (eco).
- El instrumento de Mauro siempre a mano.

### Subtext beats
- Amar = peligro para el sumidero; Mauro **sabe** lo que Bruno es y por qué.
- Seeds: `el-blanco-frio`(echo 11), `el-tutor-tiene-agenda`(echo 11), `naturaleza-de-bruno`(eco).

### Transition out
Bruno se aparta de Mauro por miedo a algo que no sabe nombrar.

## Chapter 12
- **Title:** La grieta
- **POV:** Bruno
- **Where / when:** Altaluz — preludio del vuelco
- **Target words:** 8000-12000
- **Function in the act:** la grieta previa al midpoint; preparar el sentido del 13.

### Plot beats
- Algo que Bruno creía de Altaluz/Mauro no encaja: una contradicción pequeña pero nítida.
- Un eco mudo del Blanco Falso: la "ascensión" no cuadra con lo que Bruno ha visto del color.
- Bruno empieza a mirar el lugar con otros ojos.

### Texture beats
- La luz dura que no deja sombra, ahora notada como rareza, no como belleza.
- Un detalle del lugar que delata su función (de escaparate, no de refugio).

### Subtext beats
- El lector al borde de "el refugio es escaparate" sin nombrarlo.
- **Bucle pequeño cerrado en el libro:** la duda de Bruno sobre la "ascensión" cuaja en una
  conclusión propia y firme —esa historia **no cuadra** con el color que ha visto— sin emparejar
  jamás su blanco con el Blanco Falso. Es un reconocimiento cerrado (no un misterio nuevo),
  un segundo lazo anudado antes del clímax.
- Seeds: `el-blanco-falso`(echo 12, turno de reconocimiento en el libro).

### Transition out
Bruno hace a Mauro una pregunta cuya respuesta lo va a cambiar todo.

## Chapter 13  — **MIDPOINT (el vuelco)**
- **Title:** La luz que no deja sombra
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** **EL VUELCO.** El refugio es escaparate; la mirada de Mauro
  **mide, no acompaña**. Ser visto se invierte en ser expuesto. Es también el **único cierre
  de bucle en la mitad del libro**: una creencia que el lector sostuvo desde el cap 1 (Bruno
  a salvo, por fin visto con amor) se da vuelta y **queda cerrada** — no un hilo más abierto,
  sino un lazo que se anuda al revés. Tratarlo con peso de payoff (`altaluz-es-escaparate` →
  suspected), para que el libro no se sienta "solo resuelve al final".

### Plot beats
- La respuesta/escena revela Altaluz como "la luz que no deja nada en sombra": el mejor
  escondite **y** el mejor escaparate.
- Bruno comprende que la mirada de Mauro —la que creyó amor— mide.
- Ser visto = ser expuesto; lo que creyó salvación es exhibición.

### Texture beats
- La luz total, sin sombra donde esconderse; la sensación física de estar a la vista.
- El instrumento de Mauro, ahora visto como lo que es.

### Subtext beats
- El lector cruza a **suspected** sobre `naturaleza-de-bruno`; se invierte el sentido de
  todo lo anterior, sin confirmar nada.
- Seeds: `altaluz-es-escaparate`(vuelco → suspected), `el-tutor-tiene-agenda`(vuelco),
  `naturaleza-de-bruno`(→ suspected).

### Transition out
Bruno ya no está a salvo en ninguna parte. Lo han estado mirando todo el tiempo.

## Chapter 14
- **Title:** El carro vuelve
- **POV:** Olmo
- **Where / when:** aldea verde / el camino, en paralelo
- **Target words:** 8000-12000
- **Function in the act:** subplot A toca la trama principal; el agravio madura.

### Plot beats
- Vuelve el convoy a las aldeas verdes; el agravio crece, ahora con nombres.
- El complementario verde-magenta vuelve a asomar (eco), todavía sin que nadie lo formule.
- Punto de contacto: lo que se cosecha abajo alimenta la luz de arriba (Altaluz).

### Texture beats
- El campo más gris; la ausencia de los jóvenes como un hueco en el paisaje.
- El rencor que cuaja en gestos pequeños.

### Subtext beats
- La subtrama se enhebra con la principal; el lector ve la máquina entera sin que se explique.
- Seeds: `el-convoy-jovenes`(echo 14), `el-complementario-verde-magenta`(echo 14).

### Transition out
El rencor verde sube por el mismo camino que subió Bruno.

## Chapter 15
- **Title:** Más gris
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** la tensión sube; ecos densos del despojo; el pozo pide más.

### Plot beats
- El mundo se siente más gris; Bruno nota el polvo, el prisma, el coste por todas partes.
- El hueco del pecho "pide" con más fuerza (eco).
- Se le cae otro recuerdo (el caldo, eco), casi invisible.

### Texture beats
- El despojo cotidiano acumulado; el zumbido del vidrio; el frío del blanco.
- La vega recordada ("antes de aquí se oía crecer el verde desde el camino").

### Subtext beats
- El lector empieza a temer lo que el pozo pide.
- Seeds: `el-polvo-gris`(echo 15), `el-prisma-lee-drena`(echo 15), `el-caldo`(echo 15),
  `el-blanco-frio`(echo 15), `el-pozo-que-pide`(echo 15), `el-negro-y-el-drenaje`(echo 15).

### Transition out
Se anuncia una inspección en Altaluz.

## Chapter 16
- **Title:** El casi-test
- **POV:** Cándido
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el near-miss del complementario; Cándido casi acierta y no.

### Plot beats
- Cándido inspecciona Altaluz; aplica el test del complementario a los sospechosos.
- Llega a Bruno; leído como marrón, el test **debería** dar limpio (el complementario del
  marrón no fuerza nada en un marrón). Pero bajo la tensión del complementario proyectado, el
  **tell del blanco frío parpadea** un instante —un frío donde no debería haberlo—: eso, no
  el marrón, es lo que **casi** delata a Bruno.
- Lo que lo salva: Cándido lee marrón limpio y atribuye el parpadeo a **error de instrumento**
  (deriva de calibración, un cristal frío) y no a una presa. El camuflaje marrón lo respalda:
  no busca un frío en un marrón. Archiva la anomalía sin nombre; se va sin nombrar a Bruno.

### Texture beats
- El ritual del test; los cristales; la paciencia del cazador.
- El sudor de Bruno bajo la mirada de cuentas.

### Subtext beats
- Tope **suspected**, POV-pureza: Cándido no confirma; ironía dramática máxima.
- El near-miss es intrínseco: lo que casi acierta es el **parpadeo del blanco frío** bajo el
  complementario (no una casualidad externa); lo que salva es la lectura marrón + el sesgo del
  cazador (lo achaca al instrumento).
- Seeds: `el-cazador-casi-test`(near-miss 16), `borrarse-como-el-padre`(echo 16),
  `el-blanco-frio`(parpadeo bajo complementario, rozado sin nombrar).

### Transition out
Cándido se va, pero anota Altaluz en su reloj.

## Chapter 17
- **Title:** Lo que sabe Mauro
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** aftermath; Mauro protege de un modo que delata que **sabe**.

### Plot beats
- Pasó el test; Bruno respira, pero el cerco no se abrió, se aplazó.
- Mauro lo protege de la inspección de un modo que confirma (al lector) que sabe lo que es.
- Bruno intuye que Mauro lo oculta a propósito.

### Texture beats
- La conversación a media luz; el instrumento guardado a propósito.
- El gesto de las dos manos, ahora ambiguo.

### Subtext beats
- `el-tutor-tiene-agenda`(echo 17); Bruno empieza a temer a quien ama.
- Mauro protege a Bruno de la inspección con un conocimiento del rito que solo tiene quien
  vino de dentro (`las-manos-del-lector` eco 17).
- Seeds: `el-tutor-tiene-agenda`(echo 17), `las-manos-del-lector`(echo 17).

### Transition out
Bruno entiende que su sola presencia es peligro para Mauro.

## Chapter 18
- **Title:** Ser visto es ser cazado
- **POV:** Bruno
- **Where / when:** Altaluz — cierre de acto 2 / umbral del all-is-lost
- **Target words:** 8000-12000
- **Function in the act:** **all-is-lost** — Bruno comprende que él es la amenaza para el
  único que lo vio.

### Plot beats
- Un golpe (la caza se acerca, o una pista que delata el vínculo) deja a Bruno sin suelo.
- Comprende: ser visto = ser cazado; su vínculo con Mauro los pone a ambos en peligro.
- Decide alejarse para proteger a Mauro — y no puede.

### Texture beats
- La noche de Altaluz; el miedo concreto; el pozo que pide en el silencio.

### Subtext beats
- El Need (ser visto) se ha vuelto su condena.
- Seeds: `naturaleza-de-bruno`(→ suspected), `el-cazador-casi-test`(echo 18, el cerco baja),
  `el-pozo-que-pide`(echo 18, el pozo en el silencio).

### Transition out
El cerco baja sobre Altaluz; no hay salida limpia.

# Act 3 — Clímax y resolución (ch 19-24)

> El cerco se cierra con un trigger **intrínseco** (el método de Cándido, sembrado en
> 10/16); ningún salvador externo. La única salida es el pecado que el sistema cazaba.

## Chapter 19
- **Title:** El cerco
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el low del all-is-lost; abre el hilo del padre (Libro II);
  consuelo inútil del santo.

### Plot beats
- Cándido vuelve / la caza estrecha (eco); Altaluz deja de ser escondite.
- En el desorden, aflora un **rastro del padre** (objeto/marca/nombre): Bruno lo guarda sin
  entenderlo.
- La moneda del santo vuelve como consuelo inútil.

### Texture beats
- La presión de la caza; el rastro del padre, concreto y mudo.
- Las monedas del santo, ahora amargas para el lector (no para Bruno).

### Subtext beats
- El low; se abre el Libro II (el linaje, todavía a tope `sensed`).
- Seeds: `el-cazador-casi-test`(echo 19), `rastro-del-padre`(plant 19), `la-moneda-del-santo`(echo 19).

### Transition out
El lazo se cierra; queda una sola salida, impensable.

## Chapter 20
- **Title:** Sin salida
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** la trampa **intrínseca** de Cándido cierra sola (trigger sembrado
  en 10/16) y, sin salida, el pozo del pecho "pide" con fuerza física mientras Mauro empieza
  a interponerse. (Fusión de los antiguos caps 20+21: un solo cierre del cerco.)

### Plot beats
- El método de Cándido (ya sembrado) cierra el cerco sin salvador ni delator externo; el
  test inminente que esta vez sí acertaría.
- Mauro y Bruno acorralados; las vías de escape se agotan una a una.
- El vacío del pecho de Bruno "pide" con fuerza física, ya casi como opción consciente.
- Mauro decide interponerse (empieza su decision moment); se forma la opción imposible:
  para salvar a Mauro hace falta lo que Bruno es.

### Texture beats
- El espacio que se cierra; la luz que no deja sombra, ahora jaula.
- El cuerpo de Bruno tirando hacia el vacío; el frío; la inminencia.

### Subtext beats
- El trigger es intrínseco; nada llega de fuera a salvarlos (no hay caballo, ni tormenta).
  El pozo, casi consciente; el tutor empieza a entregarse.
- Seeds: `el-cazador-casi-test`(trigger/cierre 20), `el-pozo-que-pide`(echo 20).

### Transition out
Mauro se pone entre Bruno y la caza.

## Chapter 21
- **Title:** El borde
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** preparar el apex; la única salida es drenar hacia arriba — a Mauro.

### Plot beats
- Mauro se interpone para proteger a Bruno, sabiendo el riesgo (su decision moment) — y al
  interponerse queda **piel con piel** con él.
- Bruno entiende que el drenaje que salva es **por contacto** (Ley 3): el cazador y la
  multitud coloreada quedan fuera de alcance —a distancia solo roza el fondo, y no basta—;
  el único cuerpo que tiene encima, y del que por tanto tomaría, es Mauro. La interposición
  lo ha convertido en la única presa posible.
- El instante antes de la decisión irreversible.

### Texture beats
- El contacto; el color de Mauro **contra la piel**, no solo al alcance de la mano; la
  respiración de los dos.

### Subtext beats
- El Need y la naturaleza a punto de colisionar; la salida barata (drenar al cazador, o al
  gentío) está foreclosed por el contacto — no hay escapatoria que no pase por Mauro.
- Seeds: `naturaleza-de-bruno`(→ suspected), `el-negro-y-el-drenaje`(echo 21), `el-pozo-que-pide`(echo 21).

### Transition out
Bruno toma, por primera vez a conciencia.

## Chapter 22  — **CLÍMAX**
- **Title:** El que por fin lo vio
- **POV:** Bruno
- **Where / when:** Altaluz
- **Target words:** 8000-12000
- **Function in the act:** el **primer drenaje deliberado** agrisa a Mauro —al único que lo
  vio— mientras intenta protegerlo. El pecado original.

### Plot beats
- Bruno drena, deliberadamente, para salvar a Mauro de la caza.
- Drenar hacia arriba significa drenar a Mauro: lo agrisa.
- Los ojos de Mauro suben hacia Bruno —por fin— y se apagan en el mismo gesto.
- Mauro queda **gris, hueco, pero vivo**. El cerco se rompe, a ese precio.

### Texture beats
- El polvo gris saliendo de Mauro; el hueco del pecho de Bruno saciado; la luz fría.

### Subtext beats
- El Need cumplido y destruido en el mismo acto: su pecado original.
- Seeds: `el-polvo-gris`(payoff sobre Mauro), `el-pozo-que-pide`(payoff), `ojos-que-no-suben`(payoff),
  `el-negro-y-el-drenaje`(payoff — el mismo gesto, ahora suyo),
  `el-tutor-agrisado`(intro — gris pero vivo; restaurable se siembra, no se confirma),
  `naturaleza-de-bruno`(tope suspected).

### Transition out
Bruno con el cuerpo gris de Mauro entre las manos. Lo mira, y ya no lo ven.

## Chapter 23
- **Title:** Lo que queda
- **POV:** Bruno
- **Where / when:** Altaluz, después
- **Target words:** 8000-12000
- **Function in the act:** aftermath; erosión **visible** = inocencia perdida; la de memoria,
  oculta.

### Plot beats
- Bruno con Mauro agrisado; el después inmediato.
- La caza se ha roto/desviado, pero todo lo ganado es ceniza.
- Bruno no recuerda algo que debería (el caldo, eco mudo) y no lo nota.

### Texture beats
- El silencio; el cuerpo gris; el frío que queda donde hubo una mirada.

### Subtext beats
- La inocencia perdida es visible; la erosión de memoria sigue oculta al lector.
- Seeds: `el-caldo`(echo mudo 23), `el-coste-del-don`(implícito).

### Transition out
Bruno recoge lo poco suyo, y el rastro del padre.

## Chapter 24
- **Title:** Solo, arriba
- **POV:** Bruno
- **Where / when:** Altaluz → el camino de bajada — cierre del Libro I
- **Target words:** 8000-12000
- **Function in the act:** resolución; Bruno entra al Libro II solo y culpable; quedan vivos
  los hilos de la saga.

### Plot beats
- Bruno parte solo, culpable de haber apagado al único que lo vio.
- Guarda el rastro del padre (eco): el hilo del Libro II.
- Quedan vivos el agravio verde y el cazador rondando el mito del traedor-de-gris.
- Imagen final: el último hilo humano frente al despojo.

### Texture beats
- El camino de bajada; el paisaje apagado; las campanas.
- El hilo verde guardado, su único color de hechura humana.

### Subtext beats
- Tragedia sorda; el lector vigila una desaparición, no anima una subida.
- Seeds: `rastro-del-padre`(echo 24), `el-telar-hilo-verde`(echo 24).

### Transition out
Bruno baja hacia un mundo que lo cazaría si supiera qué es, llevándose el polvo de quien
lo vio. (Fin del Libro I.)
